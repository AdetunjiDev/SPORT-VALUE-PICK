...
//
// Soft-deadline (~8s) so we return a friendly retry page before Netlify's
// hard ~10s kill turns into "This function has crashed".
// =====================================================================
...
/** Leave headroom under Netlify's ~10s sync limit. */
...
export default async (request: Request, _context: Context): Promise<Response> => {
  try {
    ...
    const deadline = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new SoftTimeoutError(SOFT_DEADLINE_MS)), SOFT_DEADLINE_MS);
    });

    return await Promise.race([work, deadline]);
  } catch (err) {
    if (err instanceof SoftTimeoutError) {
      console.warn("[netlify/app] soft-timeout", err.message);
      try {
        const path = new URL(request.url).pathname + new URL(request.url).search;
        return softTimeoutHtml(path || "/");
      } catch {
        return softTimeoutHtml("/");
      }
    }
